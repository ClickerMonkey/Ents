 ...
 * traversed through an {@link Iterator} or manually through {@link #size()} 
 * and {@link #get(int)}.
 * 
 ...
 * internal array >= 10,000. To get around this problem the stop() method 
 * should be used to stop iteration or after iteration to clean up entities 
 * referenced in the internal array AND you should also avoid allocating these 
 * filters during game play, rather create them in the beginning and reuse them 
 * as much as possible.
 * 
...
public abstract class EntityFilter implements Iterator<Entity>, Iterable<Entity> 
{
	...
	/**
	 * Determines whether the given Entity is valid. If true this Entity is
	 * added to the internal array and may be iterated over with the 
	 * {@link #iterator()} method.
	 * 
	 * @param e
	 * 		The entity to validate.
	 * @return
	 * 		True if the entity belongs in this filter, otherwise false.
	 */
	...
	/**
	 * @return
	 * 		The number of entities in the filter.
	 */
	public int size()
	{
		return stackSize;
	...
	/**
	 * Stops the filter by clearing the internal array and setting the
	 * size and iteration cursor to zero.
	 */
	public void stop()
	{
		while (--stackSize >= 0)
	...
	/**
	 * Resets this filter by stopping it and filtering it with all valid
	 * entities found within the root Entity passed in.
	 * 
	 * @param root
	 * 		The root entity to fill this filter from.
	 * @return
	 * 		The reference to this filter.
	 */
	...
	/**
	 * Returns the reference to this iterator, used when this filter
	 * is used in for-each loops.
	 */
